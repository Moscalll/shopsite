...
if (!BEFORE || !AFTER) {
  console.error('Usage: node scripts/perf/compare-perf-results.mjs <before.json> <after.json>');
  process.exit(1);
}
...
async function main() {
  ...
  const rows = targets.map((target) => {
    const metrics = ['fcpP50', 'lcpP50', 'clsP50', 'loadTimeP50', 'avgImageCount', 'avgImageSizeKB'];
    const row = { target };
    for (const metric of metrics) {
      const b = getSummary(before, target, metric);
      const a = getSummary(after, target, metric);
      row[metric] = { before: b, after: a, changePct: pct(b, a) };
    }
    return row;
  });
  ...
  for (const row of rows) {
    ...
    for (const [metric, value] of Object.entries(row)) {
      ...
      console.log(`  ${metric}: before=${fmt(value.before)} after=${fmt(value.after)} change=${fmt(value.changePct)}%`);
    }
    console.log('');
  }
